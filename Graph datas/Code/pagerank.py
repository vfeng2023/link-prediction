...
def randwalkwrestart(edges,c=0.15):
    """
    ...
    invComp = np.linalg.pinv(np.identity(N)-c*P)
    M = (1-c) * invComp
    return M
...
def AUC(edgeMat:"scipy.sparse.coo_matrix",n=1000) -> float:
    """
...
times they have the same score, the AUC value is
    ...
    idxtest,idxtrain = split(edgeMat)
    print(len(idxtest),len(idxtrain))
    traindata = [1 for _ in range(len(idxtrain))]
    trainrow = [edgeMat.row[r] for r in idxtrain]
    traincol = [edgeMat.col[c] for c in idxtrain]
    trainMat = sparse.coo_matrix((traindata,(trainrow,traincol)),shape=denseEdge.shape).toarray()
    transMat,_ = pagerank(trainMat)# pagerank(denseEdge)# pagerank(trainMat)
    print(transMat)
    # print(pagerank(adjMat)[0])
    # print(transMat)
    for owo in range(n):
        # pick random edge pair
        k = random.choice(idxtest)
        source,target = edgeMat.row[k],edgeMat.col[k]
        # choose nonexistent link
        n1,n2 = random.sample(range(denseEdge.shape[0]),2)
        while denseEdge[n1,n2] == 1:
            n1, n2 = random.sample(range(denseEdge.shape[0]),2)
        # print(transMat)
        # do the score thing
...
# read matrix
filename = "testgraph.mtx"# input("Graph filename: ")
...
# print(avg/10)
print(AUC(adjMat))
# def precision(adjMat,transMat,L=100):
#     """
